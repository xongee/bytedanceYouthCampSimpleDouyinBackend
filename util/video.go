...
import (
	"bytedanceYouthCampSimpleDouyinBackend/cache"
...
func SaveImageFromVideo(name string, isDebug bool) error {
	...
	v2i.InputPath = filepath.Join(config.Info.StaticSourcePath, name+".mp4")
	v2i.OutputPath = filepath.Join(config.Info.StaticSourcePath, name+".jpg")
	v2i.FrameCount = 1
